...
function getRequiredEnvVar(value: string | undefined, name: string, fallback?: string): string {
...
function createConfig(): AppConfig {
  ...
  const environment = getRequiredEnvVar(ENVIRONMENT, 'ENVIRONMENT', NODE_ENV || 'development') as AppConfig['environment'];
  ...
  if (!['development', 'staging', 'production'].includes(environment)) {
    throw new ConfigError(`Invalid environment: ${environment}. Must be 'development', 'staging', or 'production'`);
  }
